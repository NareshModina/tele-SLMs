...
Why packing matters:
    TeleSpec-Data has median ~500 tokens/document.
    Without packing, most of each forward pass is wasted on padding tokens.
...
Output:
    {output}/train/    HuggingFace Arrow dataset — packed blocks
    {output}/eval/     HuggingFace Arrow dataset — packed blocks
...
def tokenize_and_pack(model_name: str, output_path: str,
    ...
    # Suppress "sequence longer than max_length" warnings — intentional,
    # since we pack rather than truncate. The warning fires before packing.
