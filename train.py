...
Outputs:
    ./checkpoints/SmolLM-TS-135M/   weights + tokenizer
    ./checkpoints/SmolLM-TS-135M/trainer_state.json
"""
...
def checkpoint_dir(model_name: str) -> str:
    name = C.get_model_name(model_name)
    return os.path.join(C.CHECKPOINTS_DIR, name)
...
def main():
    ...
    parser.add_argument("--tokenized-path",
                        default=None,
                        dest="tokenized_path",
                        help="Path to pre-tokenized dataset (default: auto from config per model family)")
    ...
    # Resolve model-specific defaults after parse_args — args.model is now available
    if args.tokenized_path is None:
        args.tokenized_path = C.get_tokenized_dir(args.model)
    if args.lr is None:
        args.lr = C.get_lr(args.model)
    if args.per_device_bs is None:
